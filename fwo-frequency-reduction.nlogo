...
globals [
  bonk
  vectors ; the original vectors, unmodified by the reducing effect
  tokens ; the tokens associated with the vectors
  frequencies ; the frequencies associated with the vectors
  percentiles ; the percentiels associated with the words (computed from entire CGN words list)
...
to setup
  ...
  ; I spawn the turtles in order to have some visualisation
  ...
  ; For each of the turtles, we set the unmodified vocabulary
  ask turtles [
...
to-report compute-cumulative [ freq-list ]
  let result []
  let total_sum 0

  foreach freq-list [ f ->
    set total_sum  (total_sum  + f)
    set result lput total_sum result
  ]
  report result
end

to-report find-weighted-index [ value ]
  let i 0
  while [i < length cumulative-frequencies ] [
    if value < item i cumulative-frequencies [ report i ]
    set i i + 1
  ]
  report (length cumulative-frequencies - 1)  ; Fallback in case of rounding errors
end

to-report weighted-random-index
  let r random-float total-frequency
  let index (find-weighted-index r)
  report index
end

to-report token-reduction-prior [ percentile ]
  report 1
end
...
to load-vectors
  ...
  ; First column of the TSV are token names
  set tokens map [inner-list -> item 0 inner-list] matrix
  ; Second column are raw frequencies
  set frequencies map [inner-list -> item 1 inner-list] matrix
  ; Third column are percentiles
  set percentiles map [inner-list -> item 2 inner-list] matrix
  ...
  set total-frequency last cumulative-frequencies
end
@#$#@#$#@
GRAPHICS-WINDOW
220
...
Polygon -1 true false 105 30 90 45 90 90 105 105 105 120 75 90 75 45 105 15
Polygon -1 true false 75 90 75 105 45 75 45 60 75 30 75 45 60 60 60 75
Polygon -1 true false 45 90 45 75 30 60 45 45 45 30 15 60
...
Polygon -7500403 true true 105 90 60 150 75 180 135 105
Polygon -7500403 true true 195 90 240 150 225 180 165 105
Polygon -1 true false 180 75 150 75 195 105 270 105 285 75 285 45 270 15 195 15 180 45 180 60
