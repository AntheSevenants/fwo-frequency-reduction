...
[todo uitleggen hoe sprekers en hoorders geselecteerd worden]

The core of our simulation is a "language game" [todo]. In a language game, a speaker and a hearer agent perform a joint task rooted in language, the execution of this task naturally exerting some influence on the way language is used or organised. In our simulation, the goal of the language game is for the speaker to communicate a specific construction to the hearer, which the hearer then has to understand successfully. The exact course of the language game is as follows:

**Speaker**

1. The speaker chooses a construction to utter from the $N$ constructions available. The probability of each construction is influenced by the Zipfian A-curve, discussed in @sec-preconditions, to mirror the frequency imbalance inherent to natural language.
...
Uitleggen hoe we gaan werken. Met name: uitproberen en kijken naar bepaalde metrieken. Onze verwachtingen op voorhand oplijsten (welk gedrag moet het model vertonen?). Bijstellen indien nodig.

Indien gedrag op punt: uitgaan van Ockam's razor. Dan één voor één de requirements weghalen om te controleren of ons model écht niet werkt zonder. Indien niet is alles in kannen en kruiken.
