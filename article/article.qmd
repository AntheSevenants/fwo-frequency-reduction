...
If we look at the results in @fig-no-shared-code-model-success, we see that there is no successful communication. Agents are unable to interpret what the other agents are saying, because they do not share the same code. This violates expected behaviour #3 (see @sec-expected-behaviour).
