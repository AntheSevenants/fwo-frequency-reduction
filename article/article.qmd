...
Usage-based theories of language generally take a very broad view on memory. Usually, memory is conceptualised as a large space that is shared by all sounds, constructions at once [i.e. @pierrehumbert_exemplar_2001]. Our decision to pre-assign memory slots according to the chosen frequency distribution seems a step backwards in this regard, especially given the idea by @kretzschmar_language_2015 [32] that Zipfian curves simply arise naturally when we take any subcategorisation of language. Therefore, we should just be able to implement a global memory for each agent, and see the chosen frequency distribution arise naturally in the agents' memories.

In principle, it is true that such a bottom-up approach is possible. However, given the restrictions in a limited computer model, we opted to take a shortcut to guarantee the cognitive plausibility of our model. While we have no doubts that in the context of real human memory, certain frequency distributions need not be presupposed, our memory of 1000 exemplars is just too small to guarantee the stability of lower-frequency constructions. If we were to use a global memory, new variants of high frequency constructions would push out exemplars of low frequency constructions too easily, leading to unreliable representations of those constructions. Because it is unfortunately computationally intractable to equip agents with memories tallying hundreds of thousands of exemplars, we opted to simply assume the presupposed effect and model the frequency distribution directly in memory.
:::
...
Because this set of models covers a wide array of models, we built an aggregation graph over the model outcomes. @fig-no-zipfian-model-aggregate-success shows the average communicative success at the end of the model runs for each lambda parameter. It is clear that the larger lambda and thus the more skewed the distribution becomes, the better the model performs. Broadly speaking, the more the distribution starts to resemble a typical Zipfian distribution, the better the model performs communicatively. We turn to the discussion to attempt to explain this behaviour.
...
This ambiguity does make it hard to assess whether the Zipfian distribution is really a "required" assumption for the reducing effect. In a strict sense, it is not, since in the right circumstances with feedback, other distributions seem to work as well. Realistically, of course, the Zipfian frequency distribution seems to work in non-feedback circumstances as well, which makes it the most generally applicable specification. In light of the principle of Occam's Razor, the specification featuring a Zipfian distribution, but no feedback, is preferable (see @sec-evaluation). In addition, the flat distribution with feedback still does not lead to a difference in reduction strength or speed, and thus does not attain all requirements outlined in @sec-expected-behaviour.[^transition]
...
More broadly, we can say that the categorisation of exemplars is shown to not be as straightforward in our simulation as is assumed theoretically. While the idea is that proximity to other exemplars should suffice for correctly categorising or understanding exemplars [i.e. @pierrehumbert_exemplar_2001], frequency effects can disrupt this process and can cause cascading confusion. Of course, our simulation does not feature any sort of context nor feedback mechanism, which could help mitigate this categorisation problem. Nevertheless, our simulations show that, at least theoretically, circumstances such as sparsity can be disruptive for frequency-sensitive, usage-based theories of language storage.
