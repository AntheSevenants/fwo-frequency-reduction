...
Pitting discriminatory force and re-entrance against each other would be impossible empirically, so implementing both and checking how each solution performs is an ideal use case for computer simulations. We implemented both mechanisms in exactly the same way. For discriminatory force, we added an additional "ceiling parameter". If the decision entropy (see @sec-priors-no-priors-model) rises above this ceiling, the exemplar is not saved into the batch memory. In the same way for re-entrance, we added a ceiling parameter for the self-comprehension parse. If the decision entropy in the self-check is above the pre-determined ceiling, any reduction is reversed. We varied the ceiling parameter for both discriminatory force and re-entrance, and tested both with priors enabled (again, see @sec-priors-no-priors-model).
