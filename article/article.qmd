...
Usage-based theories of language generally take a very broad view on memory. Usually, memory is conceptualised as a large space that is shared by all sounds, constructions at once [i.e. @pierrehumbert_exemplar_2001]. Our decision to pre-assign memory slots according to the chosen frequency distribution seems a step backwards in this regard, especially given the idea by @kretzschmar_language_2015 [32] that Zipfian curves simply arise naturally when we take any subcategorisation of language. Therefore, we should be able to implement a global memory for each agent, and see the chosen frequency distribution arise naturally in the agents' memories.

In principle, it is true that such a bottom-up approach is possible. However, given the restrictions in a limited computer model, we opted to take a shortcut to guarantee the cognitive plausibility of our model. While we have no doubts that in the context of real human memory, certain frequency distributions need not be presupposed, the limited agent memory found in simulations is just too small to guarantee the stability of lower-frequency constructions. If we were to use a global memory, new variants of high frequency constructions would push out exemplars of low frequency constructions too easily, leading to unreliable representations of those constructions. Because it is unfortunately computationally intractable to equip agents with memories tallying hundreds of thousands of exemplars, we opted to simply assume the presupposed effect and model the frequency distribution directly in memory.
::: -->
