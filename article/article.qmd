...
@fig-todo shows how successful communication is among agents in two situations: the left pane shows the overall communicative success, the right pane shows communicative success for reduced forms. It is immediately clear that when frequency knowledge is part of the interpretation process, communicative success is much higher.

Our question in @sec-introduction-frequency is more specific, however. Does the addition of frequency-based priors avoid communicative difficulties stemming from ambiguity? In order to measure ambiguity, we show "decision entropy" in @fig-todo. We define decision entropy as the entropy measured over the probability distribution resulting from the Bayesian interpretation process. If one construction dominates the distribution, entropy, and thus ambiguity, is low. However, if several outcomes are similar in outcome probability, ambiguity, and thus entropy, is high. We see that while the difference between the models with and without priors remains the same, the inclusion of priors cannot prevent communicative success from trending downwards.

To find out why this is the case, we look at the vectors themselves. @fig-todo shows the mean (median? todo) $\mu$ values in the acoustic vectors associated with the five most frequent constructions. We see that the values in most dimensions are overlapping. One by one, constructions inch towards the most sparse representations, and thus start to overlap. The inclusion of priors in the decision process helps towards a correct interpretation, but also works against it: frequency knowledge will skew interpretation towards the most frequent construction, and due to its frequency, this is typically the correct outcome. However, this also means that all constructions but the most frequent one will be miscategorised once they have taken on a highly sparse representation.

### Avoiding ambiguity: discriminatory force or re-entrance? {#sec-reentrance-model}
...
Another option avoids ambiguity on behalf of the speaker. "Re-entrance", coined by @steels_language_2003, can be understood as the application of a speaker's reception subsystem to their own linguistic utterances, to serve as a means of incomprehension prevention or repair. The concept is to be understood broadly, as it can apply "either before transmitting [...] to the hearer or after observing incomprehension or communicative failure from the part of the listener" [@steels_language_2003, 8]. The argument for using the reception system for monitoring production comes from the fact that the same brain area is active for listening to both others and oneself [@mcguire_functional_1996]. In addition, psycholinguistic experiments such as @ferreira_how_2005 show empirical evidence for "comprehension monitoring", indicating that speakers can monitor their articulatory loop and preemptively avoid errors and ambiguities. Since reduction is assumed to be the result of articulatory automation, i.e. happens during the pronunciation process itself, re-entrance cannot work as a "pre-check", and thus has to be seen as a repair mechanism that occurs as a result of incomprehension. Discriminatory force does not have the same issue, since it occurs after a finished speaking turn anyway.

Pitting discriminatory force and re-entrance against each other would be impossible empirically, so implementing both and checking how each solution performs is an ideal use case for computer simulations. We implemented both mechanisms in exactly the same way. For discriminatory force, we added an additional "ceiling parameter" $\epsilon$. If the decision entropy (see @sec-priors-no-priors-model) rises above this ceiling, the exemplar is not saved into the batch memory. In the same way for re-entrance, we added a ceiling parameter $\epsilon$ for the self-comprehension parse. If the decision entropy in the self-check is above the pre-determined ceiling, any reduction is reversed. We varied the ceiling parameter for both discriminatory force and re-entrance, and tested both with priors enabled (again, see @sec-priors-no-priors-model).

We see in @fig-todo that, as the ceiling parameters become smaller, communicative success improves. Both solutions perform roughly the same; the slight underperformance of discriminatory force can be explained by how both mechanisms work. Discriminatory force prevents ambiguous exemplars (that are likely erroneously parsed) from affecting a hearer's internal representations, but communication still fails before the mechanism kicks in. In contrast, re-entrance prevents potentially problematic exemplars from being communicated in the first place, avoiding communicative failure.

For completeness' sake, we also show in @fig-todo that more frequent constructions display starker reduction than less frequent constructions, just as is expected for the reducing effect. 

![A schematic overview of the language game played by the speaker and hearer, with re-entrance and discriminatory force. For re-entrance, the speaker checks whether the ambiguity when hearing themselves speak is not too high. If it is, reduction is reversed. Else, it is kept. For discriminatory force, the hearer checks if their parsed output was not too ambiguous. If it was, the form is not saved to batch memory. Else, it is. The new elements in the overview are indicated in teal. Both mechanisms are never active at the same time.](tikz/language_game_bayes_reentrance.svg){#fig-model_design_language_game_reentrance}

### Testing the impact of the Zipfian distribution {#sec-no-zipfian-feedback-model}

We mentioned in @sec-introduction-frequency that we wanted to find out to what extent contextual frequency information helps combat ambiguity caused by reduction. In @sec-priors-no-priors-model, we saw that including frequency-based priors helps improve communicative success, but does not solve ambiguity. There is another frequency-related influence which we have not yet tested, however: the frequency distribution of the constructions themselves. How does the way the frequency of constructions is distributed influence communication? The frequency of constructions is invariant in real life, which makes it difficult to test its influence. In our simulations, however, we *can* change the frequency distribution of constructions, and therefore check what happens when the typical Zipfian influence is gradually lost. We varied parameter $\beta$ of @eq-zipfian_distribution to change how steep the frequency drop-off is between constructions. As $\beta$ becomes smaller, the distribution begins to approach a uniform distribution, i.e. frequency differences between constructions gradually become lost. Note that we also enabled re-entrance for this series of simulations.

@fig-todo shows that, as $\beta$ becomes smaller, communicative success begins to suffer. It seems, then, that a decently skewed distribution seems relevant for maintaining communicative stability during reduction, at least in our implementation without feedback. To check whether this behaviour changes with the inclusion of feedback, we additionally implemented a feedback mechanism. At communication time, the hearer receives a feedback signal from the speaker. If their interpretation was correct, they save the exemplar heard in their batch memory. If not, they do not save anything, and communication fails (i.e. there is no further repair mechanism). The revised flow of the language game is shown schematically in @fig-model_design_language_game_reentrance_feedback. As is clear from @fig-todo, a flat distribution ($\beta = 0.01$) *with* feedback can perform just as well as a skewed distribution without feedbaack. We turn to the discussion to attempt to explain this behaviour.
...
We saw that both ambiguity avoidance mechanisms we tested, discriminatory force and re-entrance, performed similarly. While re-entrance has the advantage of avoiding more failed communicative turns, its vague definition (it can either apply during speech production, or after failed communication) makes it less stable theoretically. This issue does not arise with discriminatory force, though this mechanism has slightly worse performance in our simulations. A more elaborate simulation architecture is needed to better compare these mechanisms, but this was beyond the scope of the current study.
