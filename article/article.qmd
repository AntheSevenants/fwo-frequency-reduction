...
In this section, we go over some preconditions which we assume are necessary to model reduction in a theoretically accurate away. For this, we base ourselves on empirical research in the usage-based linguistics paradigm [as recommended by @loreto_theoretical_2010]. Usage-based linguistics views language as a complex adaptive system, i.e. a system shaped through repeated use by language users [@ellis_what_2012]. This makes it a natural fit for our agent-based simulations, which operate on the same principle.
