...
In this subsection, we look at the model behaviour of the following specification:
...
Our exponential distribution is organised according to TODO. We vary the skew in the exponential distribution from $log_{10}\left(1\right)$ to $log_{10}\left(3\right)$.

Because this set of models covers a wide array of models, we build an aggregation graph over the model outcomes. TODO shows the average communicative success for each lambda parameter at the end of the model runs. It is clear that the more skewed the distribution becomes, the better the model performs.
