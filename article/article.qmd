...
| $D$ | Number of vector dimensions | 100 |
...
We see in @fig-base-model-success that overall, communicative success in the model remains above chance level, although it does decrease. As we mentioned, it is expected that there is some trade-off between sparsity and communicative success, and the model results arguably remain within this margin. However, the confusion matrix in @fig-base-model-matrix paints a different picture: for the top 35 most frequent constructions, there is a large discrepancy between what the speaker intended (y axis) and what the hearer heard (x axis). More specifically, all frequent constructions seem to become confused with the most frequent construction. If we take English words as an illustration, this behaviour is comparable to speakers saying words like "have", "you", or "but", with hearers always hearing the most frequent English word "the". Obviously, this is a major flaw in the communication system of the agents, and it also does not correspond with realistic linguistic behaviour: reduction effects in real life do not lead to mass confusion. 
