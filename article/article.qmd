...
The second unknown revolves around an explicit link between frequency and the speaker's inclination to reduce. @bybee_phonology_2003 [11] states:
...
2. Does there need to be an explicit link between frequency and the speaker's inclination to reduce for the reducing effect to arise as expected?[^expected]
...
2. Does there need to be an explicit association between frequency and the speaker's inclination to reduce?
...
As for (2), we suppose the most minimal option and initially assume there not to be an explicit relation between frequency and the speaker's inclination to reduce. This assumption can be defended theoretically, as the "Principle of Least Effort" [@zipf_human_1949], dictates that when possible, language users will attempt to conserve as much energy as possible when speaking. Initially, then, we assume that this principle holds universally, regardless of frequency.
...
We know that (3) is essential for reducing effect behaviour, as the results for our base model (@sec-base-model) did not satisfy our expected behaviour. We cannot disable (2), as the association between frequency and the speaker's inclination to reduce was never implemented, and it also never proved necessary in the (successful) re-entrance model (@sec-reentrance-model). This leaves us with our Zipfian assumption in (1). Is it really needed to lead to the reducing effect?
