...
To find out why this is the case, we look at the vectors themselves. @fig-priors-unique-plot_energy_per_ctx_per_dim_norm_article shows the median normal distributions across agents in three dimensions of the acoustic vectors associated with the five most frequent constructions, for a random run in the simulations with priors enabled. We see that the all distributions are overlapping in exactly the same way, at both sides. The inclusion of priors in the decision process helps towards a correct interpretation: frequency knowledge will skew interpretation towards the most frequent construction, and due to its frequency, this is, of course, typically the correct outcome. However, when less frequent constructions are communicated, they will *also* be corrected towards a more frequent construction. As the representations incrementally overlap, the problem cascades. This "overlap" between constructions is expected given our model set-up: agents do not know whether they are reducing "too far", and thus their interal representations can start to "spill over" each other.
...
Overview of Zipfian distributions with various degrees of speeness, and a comparison of communicative success between models with this various steepness.
:::
