...
Nevertheless, a solution within this design is possible, on the side of the speaker. If we let the speaker reduce an acoustic representation, we first let them reflect whether they would understand the uttered form correctly themselves. If they would, communication can continue as planned. If they would not, reduction is reversed, and the unreduced form is communicated. This mechanism is named "re-entrance" by @steels-todo and TODO. TODO bespreken!!!
...
In @fig-no-half-life-per-construction, we show the "half life" period for each construction. We show how many steps in the simulation it took for every acoustic representation to reach half of its original energy. We see that very little acoustic representations reduce to half their energy in the Zipfian simulation, in contrast to the linear simulation, where many representations have indeed reduced to this point. This shows that, if frequency is assumed to be linear, the reduction effect works much faster in our simulation. In this regard, we can say that expected behaviour #2 (see @sec-expected-behaviour) is violated, since a model of reduction with assumed linear frequency does not produce any stark differences in reduction speed and extent between high and low frequency items.

The linear results put the L1 distribution of our Zipfian simulation into perspective. It seems as if the Zipfian model organically organises our representations in the most efficient way. To explain this reorganisation process, we start from our cone model (@sec-sec-cone-model). At the very bottom of the cone, only one construction can exist. Naturally, the most frequent construction takes up this space, as it is the first to reach all the way there. Less frequent constructions can divide the space slightly above the bottom of the cone, at the expense of using slightly more energy. Since the cone is wider here, several forms can co-exist. When this space fills up, the area even further up the cone is next. This area is even wider, and allows for even more constructions, again at the expense of a higher energy usage.

This layered behaviour can be identified in @fig-reentrance-model-model-l1-per-construction, as the acoustic representations seem to reorganise themselves in the shape of a staircase. The most frequent construction occupies the smallest possible representation: a vector filled with floor values ($M \times R = 100 \times 15 = 1500$). The next most frequent constructions are expressed through vectors with more acoustic information. They cannot also use the smallest possible representation, as that position has already been taken up. Therefore, they use the portion of the vector space that has the smallest total energy, yet does not generate confusion with other forms in the neighbourhood. Positions with more energy allow for more variation, which makes it so more "energetic" vector locations also feature more forms, as can be seen in the staircase. Once this phonetic space is also full, vectors will need to be expressed using even more energy, allowing for even more variation, and so on. TODO herschrijven

Dit is allemaal heel efficiënt. Dat is leuk!

- komt erop neer dat alles gewoon veel rapper neergaat
- alles plat
- met het verschil: dat gebeurt bij Zipfian sampling eigenlijk ook, maar veel trager en later
- het probleem is dat alle variatie een beetje wordt platgedrukt omdat er geen eindpunt is. nog eens over nadenken
